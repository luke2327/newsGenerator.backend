...
def result():
    ...
    filename = ''
    news_name = (datetime.now(pytz.utc) + timedelta(hours=9)).strftime('%d%H%M%S')
    dirc_name = (datetime.now(pytz.utc) + timedelta(hours=9)).strftime('%Y%m')
    ...
    doc_object_key = 'news/' + dirc_name + '/' + news_name + '.html'
    ...
    print current_time() + '[Defined data] language : ' +\
                            result.get('language_cd') +\
                            ', object_key : ' + doc_saving_key

    try:
        print current_time() + 'The file save_data will be opened.'
        file_s.filename = dirc_name + '/' + filename
        save_data = open(doc_saving_key, 'w')
        save_data.write(render_template("result.html",
                                        result = result, file_s = file_s).encode('utf-8'))
    ...
    try:
        ...
        print current_time() + 'The file data will be closed.'
        data_doc.close()
        data_img.close()
